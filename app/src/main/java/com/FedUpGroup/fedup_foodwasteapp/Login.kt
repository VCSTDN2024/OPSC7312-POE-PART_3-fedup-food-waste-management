...
class Login : AppCompatActivity() {
    ...
    private fun userLogin(loginEmail: String, loginPassword: String) {
        Log.d("LoginActivity", "Attempting to sign in with email: $loginEmail")

        mAuth.signInWithEmailAndPassword(loginEmail, loginPassword)
            .addOnCompleteListener(this) { task ->
                if (task.isSuccessful) {
                    Log.d("LoginActivity", "Sign-in successful for email: $loginEmail")

                    // Sign in success, now get the Firebase token
                    getFirebaseToken { token ->
                        if (token != null) {
                            Log.d("LoginActivity", "Firebase token retrieved successfully")

                            // Display success message to user
                            showSnackbar("Login Successful", android.R.color.holo_green_dark)

                            // Start MainActivity, passing the token if needed
                            val intent = Intent(this@Login, MainActivity::class.java)
                            intent.putExtra("jwt_token", token)
                            startActivity(intent)
                            finish()

                            Log.d("LoginActivity", "Navigated to MainActivity with token")
                        } else {
                            Log.e("LoginActivity", "Failed to retrieve Firebase token")
                            showSnackbar("Error retrieving token", android.R.color.holo_red_dark)
                        }
                    }
                } else {
                    // If sign-in fails, display a message to the user
                    Log.e("LoginActivity", "Sign-in failed for email: $loginEmail", task.exception)
